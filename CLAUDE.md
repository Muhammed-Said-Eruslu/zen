...
- `index.html` — home page with the full-screen hero
...
- `stone-texture.js` — Canvas API background texture generator (currently commented out in `index.html`)
...
- **Typography** — Cormorant Garamond (body serif), Cinzel (headings/nav), Poppins (UI labels), Playfair Display / Bodoni Moda (home hero only). All `font-size` values use `clamp()` for fluid scaling.
...
- `stone-texture.js` is a purely decorative canvas background; it only activates on pages with `body.home-page` and is currently disabled (commented out).
